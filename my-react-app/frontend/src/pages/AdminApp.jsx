...
export default function AdminApp({ onLogout }) {
  ...
  useEffect(() => {
    ...
    fetch(`${BASE_URL}/instructors`)
      .then((res) => res.json())
      .then((data) => setInstructorsData(data.data || []));
  ...
  const handleAddInstructor = () =>
    alert("Tính năng thêm giảng viên mới đang được mở!");
            ...
            <div key={i} className="col-3">
              <div className="card border-0 shadow-sm rounded-4 p-3 bg-white h-100">
                ...
                <div className="d-flex justify-content-between align-items-end">
                  ...
                  <span
                    className={`badge rounded-pill px-2 py-1 small ${m.isUp ? "bg-success bg-opacity-10 text-success" : "bg-danger bg-opacity-10 text-danger"}`}
                  >
                    <i
                      className={`fa-solid ${m.isUp ? "fa-arrow-trend-up" : "fa-arrow-trend-down"} me-1`}
                    ></i>{" "}
                    {m.trend}
                  </span>
                </div>
        ...
        <div className="row g-3">
          <div className="col-8">
            <div className="card border-0 shadow-sm rounded-4 p-4 bg-white h-100">
              ...
              <div
                className="d-flex align-items-end justify-content-between flex-grow-1"
                style={{ height: "200px" }}
              >
                ...
                {dashboardData.topCourses.map((c, i) => (
                  <div key={i}>
                    <div className="d-flex justify-content-between small mb-1">
                      ...
                      <span className="text-muted">{c.students}</span>
                    </div>
                    <div
      ...
      <div className="p-4">
        <div className="d-flex justify-content-between align-items-center mb-4">
          ...
          <button
            ...
            className="btn text-white fw-bold px-3 rounded-3 shadow-sm"
                  ...
                  filteredCourses.map((c) => (
                    <tr key={c.id} className="border-bottom">
                      <td className="ps-4 py-3">
                        <h6 className="fw-bold m-0 text-dark">{c.name}</h6>
                      ...
                      <td>
                        <span className="badge rounded-pill px-2 py-1 bg-success bg-opacity-10 text-success">
                          • {c.status}
                        </span>
                      </td>
                      <td className="pe-4 text-end">
                        <button
                          ...
                          className="btn btn-sm btn-light me-2 rounded-circle"
                        >
        ...
        {showModal && (
          ...
          >
            <div
              className="bg-white p-4 rounded-4 w-100 shadow-lg"
              style={{ maxWidth: "450px" }}
            >
              <div className="d-flex justify-content-between align-items-center mb-4">
                <h5 className="fw-bold m-0">{modalTitle}</h5>
                <i
                  className="fa-solid fa-xmark fs-5 cursor-pointer text-muted"
  ...
  const InstructorsTab = () => {
    ...
    return (
      <div className="p-4">
        <div className="d-flex justify-content-between align-items-center mb-4">
          <h4 className="fw-bold text-dark m-0">Instructors</h4>
          <div className="d-flex gap-2">
            <button
              onClick={handleFilter}
              className="btn btn-white border fw-bold rounded-3 shadow-sm"
            >
              ...
              <tbody>
                {filteredStudents.map((s) => (
                  <tr key={s.id} className="border-bottom">
                    ...
                    <td className="fw-bold text-center text-dark small">
                      {s.coursesEnrolled}
                    </td>
                    <td>
      ...
      >
        ...
        <div className="px-3 flex-grow-1 mt-2 d-flex flex-column gap-1">
          <button
            ...
            className={`btn text-start border-0 fw-semibold px-3 py-2 rounded-3 w-100 ${activeTab === "dashboard" ? "text-primary" : "text-muted"}`}
            style={{
              backgroundColor:
                activeTab === "dashboard" ? "#f5f3ff" : "transparent",
          ...
          <button
            ...
            className={`btn text-start border-0 fw-semibold px-3 py-2 rounded-3 w-100 ${activeTab === "courses" ? "text-primary" : "text-muted"}`}
            style={{
              backgroundColor:
                activeTab === "courses" ? "#f5f3ff" : "transparent",
              color: activeTab === "courses" ? "#3525cd" : "",
            }}
          >
            <i className="fa-solid fa-book-open me-3"></i>Courses
          </button>
          <button
            onClick={() => setActiveTab("students")}
            className={`btn text-start border-0 fw-semibold px-3 py-2 rounded-3 w-100 ${activeTab === "students" ? "text-primary" : "text-muted"}`}
            style={{
              backgroundColor:
                activeTab === "students" ? "#f5f3ff" : "transparent",
          ...
          <button
            ...
            className={`btn text-start border-0 fw-semibold px-3 py-2 rounded-3 w-100 ${activeTab === "instructors" ? "text-primary" : "text-muted"}`}
            style={{
              backgroundColor:
                activeTab === "instructors" ? "#f5f3ff" : "transparent",
